...
if(isset($_POST['add_class'])) {
    ...
    if($name !== '') {
        $stmt = mysqli_prepare($conn, "INSERT INTO classes (name, schedule, teacher_id) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "ssi", $name, $schedule, $teacher_id);
...
if(isset($_GET['delete'])) {
    ...
    // Ensure the class belongs to the teacher before deleting
    $check_stmt = mysqli_prepare($conn, "SELECT id FROM classes WHERE id = ? AND teacher_id = ?");
    mysqli_stmt_bind_param($check_stmt, "ii", $id, $teacher_id);
...
if(isset($_POST['edit_class'])) {
    ...
    $schedule = trim($_POST['schedule']);
    if($name !== '') {
        // Ensure the class belongs to the teacher before updating
        $check_stmt = mysqli_prepare($conn, "SELECT id FROM classes WHERE id = ? AND teacher_id = ?");
        mysqli_stmt_bind_param($check_stmt, "ii", $id, $teacher_id);
        mysqli_stmt_execute($check_stmt);
        $check_result = mysqli_stmt_get_result($check_stmt);
...
$classes = [];
$stmt = mysqli_prepare($conn, "SELECT * FROM classes WHERE teacher_id = ? ORDER BY name ASC");
